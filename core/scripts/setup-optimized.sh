#!/usr/bin/env bash
# AGI-System Installations-Skript (Optimierte Version)
# 
...
LOG_FILE="/tmp/agi-system-setup-$(date +%Y%m%d%H%M%S).log"
START_TIME=$(date +%s)

# Benötigte Abhängigkeiten
DEPENDENCIES=("git" "bash")
OPTIONAL_DEPENDENCIES=("node" "npm" "git-crypt")
...
show_banner() {
    ...
    echo "                                        "
    echo -e "AGI-System Installation (Optimiert)${NC}"
    echo -e "${GRAY}$(date)${NC}"
    echo
}
...
check_permissions() {
    ...
    if [[ -z "$email" ]]; then
        log "WARNING" "Keine E-Mail angegeben. Standardinstallation wird durchgeführt."
        return 1
    fi
    ...
    if [[ ! -f "$permission_file" ]]; then
        log "WARNING" "Keine Berechtigungsdatei gefunden. Standardinstallation wird durchgeführt."
        return 1
    fi
    
    # Prüfe, ob Benutzer vorhanden ist
    if grep -q "\"email\": \"$email\"" "$permission_file"; then
        log "SUCCESS" "Benutzer mit E-Mail $email gefunden."
        ...
        log "INFO" "Zugriffsebene: $access_level"
        
        # Zugriffsebene exportieren
        export USER_ACCESS_LEVEL="$access_level"
        return 0
    else
        log "WARNING" "Keine Berechtigung für E-Mail $email gefunden."
        log "WARNING" "Standardinstallation wird durchgeführt."
        return 1
    fi
}
...
create_directories() {
    ...
    # Verzeichnisse parallel erstellen mit einem Subprozess
    (
        for dir in "${dirs[@]}"; do
            mkdir -p "$dir" &
        done
...
create_user_profile() {
    ...
    log "INFO" "Erstelle Benutzerprofil..."
    
    # Überprüfen, ob entschlüsselte Profile verfügbar sind
    if [[ -f "$REPO_ROOT/.git-crypt/user-profiles/$email.about" ]]; then
        log "INFO" "Importiere bestehendes Benutzerprofil für $email..."
...
EOF
    ...
    log "SUCCESS" "Benutzerprofil erstellt."
    return 0
}

# Funktion zum Einrichten der Umgebungsvariablen
setup_environment() {
    log "INFO" "Richte Umgebungsvariablen ein..."
    
    # Überprüfen, ob entschlüsselte API-Schlüssel verfügbar sind
...
EOF
    ...
    log "SUCCESS" "Umgebungsvariablen eingerichtet."
    return 0
}
...
# Funktion zum Import der MCP-Server-Konfiguration
import_mcp_config() {
    if [[ -f "$REPO_ROOT/.git-crypt/mcpservers-full.json" ]]; then
        ...
        log "SUCCESS" "MCP-Server-Konfiguration importiert."
    else
        log "INFO" "Keine erweiterte MCP-Server-Konfiguration verfügbar. Standardkonfiguration wird verwendet."
    fi
...
summarize_installation() {
    ...
    if [[ -z "$USER_ACCESS_LEVEL" || "$USER_ACCESS_LEVEL" == "viewer" ]]; then
        echo
...
main_installation() {
    ...
    echo "# AGI-System Installation Log" > "$LOG_FILE"
    echo "# $(date)" >> "$LOG_FILE"
    ...
    # Berechtigungen prüfen (aber nicht abbrechen bei Fehlern)
    check_permissions "$email"
    ...
    # Benutzerprofil und Umgebung einrichten (parallel)
    create_user_profile "$name" "$email" &
    setup_environment &
    wait
    
    # MCP-Konfiguration importieren und Berechtigungen setzen (parallel)
    import_mcp_config &
    set_permissions &
    wait

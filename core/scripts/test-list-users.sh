...
list_users() {
    echo "INFO: Liste alle Benutzer auf..."
    ...
    if command -v jq &> /dev/null; then
        # Mit jq
        jq -r '.admin[] | "  - \(.name) (\(.email)) [seit \(.added)]"' "$PERMISSIONS_FILE" 2>/dev/null || echo "  Keine Administratoren gefunden."
    else
        # Ohne jq
    ...
    if command -v jq &> /dev/null; then
        # Mit jq
        jq -r '.contributors[] | "  - \(.name) (\(.email)) [seit \(.added)]"' "$PERMISSIONS_FILE" 2>/dev/null || echo "  Keine Mitwirkenden gefunden."
    else
        # Ohne jq
    ...
    if command -v jq &> /dev/null; then
        # Mit jq
        jq -r '.viewers[] | "  - \(.name) (\(.email)) [seit \(.added)]"' "$PERMISSIONS_FILE" 2>/dev/null || echo "  Keine Betrachter gefunden."
    else
        # Ohne jq
